...
class CompilationEngine:
    ...
    def __init__(self, input_stream, output_file):
        ...
        self.filename = output_file.split('\\')[-1].replace('.vm', '')
    ...
    def compile_class(self):
        ...
        if not self.class_name == self.filename:
            raise SyntaxError(f'expected class name: "{self.filename}".')
        tk.advance()
        tk.advance()  # "{"
    ...
    def compile_class_var_dec(self):
        ...
        while tk.curr_token != ';':
            ...
            if tk.token_type() != 'IDENTIFIER':
                raise SyntaxError(f"{tk.curr_token} isn't a valid identifer.")
            self.symbol_table.define(tk.curr_token, _type, cat)
            tk.advance()
    ...
    def compile_parameter_list(self):
        ...
        while tk.curr_token != ')':
            ...
            if tk.token_type() != 'IDENTIFIER':
                raise SyntaxError(f'{tk.curr_token} is not a valid identifer.')
            self.symbol_table.define(tk.curr_token, _type, cat)
            tk.advance()
    ...
    def compile_let_statement(self):
        ...
        if tk.token_type() != 'IDENTIFIER':
            raise SyntaxError(f"{tk.curr_token} isn't a valid Jack identifer.")
    ...
    def compile_term(self):
        ...
        if tk.token_type() == 'STRING_CONST':
            ...
            if tk.token_type() != 'IDENTIFIER':
                raise SyntaxError(f"{tk.curr_token} isn't a valid identifier.")
            var_name = tk.curr_token
            tk.advance()
            if tk.curr_token == '[':
                tk.advance()  # "["
